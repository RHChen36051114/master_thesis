...
if __name__ == "__main__" :
    ...
    while (abs(radius*2 - sideLEN)) > 0.001 :
        ...
        lat_inte, lon_inte = p1_1.find_poi_array_interval (float(specPOI[0][1]), float(specPOI[0][2]), r_act, partDEG)
    ...
    for row in specPOI :
        ...
        lat_interval, lon_interval = p1_1.find_poi_array_interval (float(row[1]), float(row[2]), r_act, partDEG)

        # return target poi img
        # [0]: # of check-ins,  [1]: accu poi MAP (seen using function),  [2][3]: center lati & longi
        # filter check-ins = 1
        if int(row[4]) > 1 :
            poiIMG.append ([int(row[4]), p1_1.accu_checkin(lat_interval, lon_interval, partDEG, poiMAP), row[1], row[2]])
        cnt += 1
